...
function loadState() {
	try {
		const raw = fs.readFileSync(path.join(__dirname, '..', 'data', 'data/mention.json'), 'utf8');
...
function saveState(state) {
	fs.writeFileSync(path.join(__dirname, '..', 'data', 'data/mention.json'), JSON.stringify(state, null, 2));
}
...
async function mentionCommand(sock, from, msg, args) {
	try {
		...
		if (!state.assetPath) {
			await sock.sendMessage(from, { text: 'Hi' }, { quoted: msg });
			return;
		}
		const assetPath = path.join(__dirname, '..', state.assetPath);
        ...
        try {
            if (state.type === 'sticker') {
                await sock.sendMessage(from, { sticker: fs.readFileSync(assetPath) }, { quoted: msg });
                return;
            }
            const payload = {};
...
async function mentionToggleCommand(sock, chatId, message, args, isOwner) {
	if (!isOwner) return sock.sendMessage(from, { text: 'Only Owner or Sudo can use this command.' }, { quoted: msg });
	const onoff = (args || '').trim().toLowerCase();
	if (!onoff || !['on','off'].includes(onoff)) {
		return sock.sendMessage(from, { text: 'Usage: .mention on|off' }, { quoted: msg });
	}
	const state = loadState();
	state.enabled = onoff === 'on';
	saveState(state);
	return sock.sendMessage(from, { text: `Mention reply ${state.enabled ? 'enabled' : 'disabled'}.` }, { quoted: msg });
}
...
async function setMentionCommand(sock, chatId, message, isOwner) {
	...
	if (!qMsg) return sock.sendMessage(from, { text: 'Reply to a message or media (sticker/image/video/audio/document).' }, { quoted: msg });
	...
	else return sock.sendMessage(from, { text: 'Unsupported. Reply to text/sticker/image/video/audio/document.' }, { quoted: msg });

	// Download or capture text
	if (type === 'text') {
		buf = Buffer.from(qMsg.conversation || qMsg.extendedTextMessage?.text || '', 'utf8');
		if (!buf.length) return sock.sendMessage(from, { text: 'Empty text.' }, { quoted: msg });
	} else {
		try {
	...
	if (buf.length > 1024 * 1024) {
		return sock.sendMessage(from, { text: 'File too large. Max 1 MB.' }, { quoted: msg });
	}
	...
	try { fs.writeFileSync(outPath, buf); } catch (e) {
		...
		return sock.sendMessage(from, { text: 'Failed to save file.' }, { quoted: msg });
	}
	...
	saveState(state);
	return sock.sendMessage(from, { text: 'Mention reply media updated.' }, { quoted: msg });
}
