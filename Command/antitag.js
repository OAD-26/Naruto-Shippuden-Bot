...
async function handleAntitagCommand(sock, chatId, userMessage, senderId, isSenderAdmin, message) {
    try {
        if (!isSenderAdmin) {
            await sock.sendMessage(from, { text: '```For Group Admins Only!```' },{quoted :message});
            return;
        }
        ...
        if (!action) {
            ...
            await sock.sendMessage(from, { text: usage },{quoted :message});
            return;
        }
        ...
        switch (action) {
            case 'on':
                ...
                if (existingConfig?.enabled) {
                    await sock.sendMessage(from, { text: '*_Antitag is already on_*' },{quoted :message});
                    return;
                }
            ...
            case 'off':
                ...
                await sock.sendMessage(from, { text: '*_Antitag has been turned OFF_*' },{quoted :message});
                break;
            ...
            default:
                await sock.sendMessage(from, { text: `*_Use ${prefix}antitag for usage._*` },{quoted :message});
        }
    } catch (error) {
        console.error('Error in antitag command:', error);
        await sock.sendMessage(from, { text: '*_Error processing antitag command_*' },{quoted :message});
    }
}
